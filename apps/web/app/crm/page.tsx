...
export default function CRMPage() {
  ...
  const [userRole, setUserRole] = React.useState<string>('superadmin');
  ...
  const availableResponsibles = useMemo(() => {
    ...
    leads.forEach(l => {
      ...
      if ((l as any).assignedToId) set.add((l as any).assignedToId);
    });
    return Array.from(set);
  }, [leads]);

  // Filter leads based on search query, responsible, tag, and scope
  const filteredLeads = useMemo(() => {
    return leads.filter(l => {
      ...
      // Scope filter: Apenas Meus
      if (scopeFilter === 'mine') {
        const isMine = (
          (userId && (l.assignedUserId === userId || (l as any).assignedToId === userId || (l as any).assignedTo?.id === userId)) ||
          (userName && (l as any).assignedTo?.name?.toLowerCase() === userName.toLowerCase()) ||
          (!userId && !l.assignedUserId && !(l as any).assignedToId)
        );
        if (!isMine) return false;
      }

      const matchesResponsible = selectedResponsible === 'all' || 
        l.assignedUserId === selectedResponsible || 
        (l as any).assignedToId === selectedResponsible;

      const matchesTag = selectedTag === 'all' || (l.tags && l.tags.includes(selectedTag));

      return matchesSearch && matchesResponsible && matchesTag;
    });
  }, [leads, searchQuery, selectedResponsible, selectedTag, scopeFilter, userId, userName]);
    ...
    <div className="space-y-5 h-full flex flex-col max-w-[1700px] mx-auto">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto justify-between lg:justify-end">
          ...
          <div className="hidden sm:flex items-center gap-3 px-3.5 py-2 rounded-2xl bg-[#111513] border border-[#222924] text-xs font-mono">
            ...
            {/* Hide global financial totals for non-OWNER/ADMIN if not filtered to mine */}
            {(isOwnerOrAdmin || scopeFilter === 'mine') && (
              <>
                <span className="text-[#222924]">|</span>
                <div className="flex items-center gap-1.5">
                  <TrendingUp className="w-3.5 h-3.5 text-[#57EF40]" />
                  <span className="text-muted-foreground">{scopeFilter === 'mine' ? 'Meu Volume:' : 'Volume Total:'}</span>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-2xl bg-[#111513] border border-[#222924] shrink-0">
        <div className="flex flex-wrap items-center gap-2.5 flex-1 min-w-[280px]">
          ...
          <div className="flex items-center gap-1.5 bg-[#18201C] border border-[#222924] rounded-xl p-1 text-xs">
        ...
        {(searchQuery || selectedTag !== 'all' || selectedResponsible !== 'all' || (isOwnerOrAdmin && scopeFilter !== 'all')) && (
          <button
            onClick={() => {
              setSearchQuery('');
              setSelectedTag('all');
              setSelectedResponsible('all');
              if (isOwnerOrAdmin) setScopeFilter('all');
            }}
